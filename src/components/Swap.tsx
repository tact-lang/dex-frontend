...
import { Card, Button, Box, Stack, Flex, Text, Input, IconButton, Link, Image, useDisclosure, InputGroup, DataList, Group, Dialog, Portal, CloseButton, HStack } from "@chakra-ui/react"
...
export default function Swap() {
  ...
  // Dialog states
  const { open: isFromTokenDialogOpen, onOpen: onFromTokenDialogOpen, onClose: onFromTokenDialogClose } = useDisclosure()
  const { open: isToTokenDialogOpen, onOpen: onToTokenDialogOpen, onClose: onToTokenDialogClose } = useDisclosure()
  const { open: isSettingsDialogOpen, onOpen: onSettingsDialogOpen, onClose: onSettingsDialogClose } = useDisclosure()
    ...
    <Card.Root variant="subtle" p={6} data-testid="swap-card">
      ...
      <Stack gap={4} align="stretch">
        <SwapSection
           ...
           token={fromToken}
           amount={fromAmount}
           onAmountChange={(value) => handleFromAmountChange({ target: { value } } as React.ChangeEvent<HTMLInputElement>)}
        ...
        <SwapSection
           ...
           token={toToken || null}
           amount={toAmount}
           onAmountChange={(value) => handleToAmountChange({ target: { value } } as React.ChangeEvent<HTMLInputElement>)}
        ...
        {toToken ? (
          <DataList.Root orientation="horizontal">
            <DataList.Item>
              ...
              <DataList.ItemValue>1 {fromToken.symbol} = {directRate} {toToken.symbol}</DataList.ItemValue>
            </DataList.Item>
            <DataList.Item>
              <DataList.ItemLabel>Reverse Price</DataList.ItemLabel>
              <DataList.ItemValue>1 {toToken.symbol} = {reverseRate} {fromToken.symbol}</DataList.ItemValue>
            </DataList.Item>
          </DataList.Root>
        ) : null}
      </Box>
